      ...
      <div className="space-y-4">
        {menuItems.map((item) => (
          <Link key={item.href} href={item.href}>
            <button className={`w-full flex items-center gap-4 p-4 rounded-xl font-bold text-xl transition ${pathname === item.href || pathname.startsWith(item.href + '/') ? 'bg-blue-600' : 'bg-gray-800 hover:bg-gray-700'}`}>
